...
TtNow, you're ready to choose a directory to delete.
...
UpgradeNeededDiskSpace = 30000000 
...
def findSmallestDirToDelete(root: Directory, spaceNeeded: int) -> tuple[str, int]:
    dirs: list[Directory] = [root]
    dirToDelete = ('', TotalDiskSpace)
    while True:
        if len(dirs) == 0: break 
        dir = dirs.pop(0)
        if dir.size >= spaceNeeded and dir.size < dirToDelete[1]: dirToDelete = (dir.name, dir.size)
        dirs += dir.directories
    return dirToDelete 
...
def main():
    for f in testData:
        ...
        needToFree = UpgradeNeededDiskSpace - freeSpace  
        print(f"used space: {usedSpace}, free space: {freeSpace}, need to free: {needToFree}")
        answer = findSmallestDirToDelete(root, needToFree)
        print(f"need to delete directory {answer[0]} with size {answer[1]}")
